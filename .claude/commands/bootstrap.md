...
### Step 3c: Generate Task List

Run `/create-tasklist` to generate `tasks.md` from the context files now in `.claude/`. It reads the PRD, workflow, infra, security, SBOM, tests, and changelog files and synthesizes a structured work queue with status tracking, phase-based categorization, and a dependency graph. This becomes the canonical shared task list for all agents and the developer.
